@extends('frontend')
@section('content')

    <section data-position="true" class="box-news" id="box-news">
        <div class="fix">
            <div class="layout">
                <div class="layout-left">
                    <article class="detail" id="data-detail">
                        <h1 class="title">Cây thuốc nam</h1>
                        <div class="data">
                            <div class="col-left">
                                <ul class="list">
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">1.Bệnh suy dinh </a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">2.Bệnh còi xương suy dinh </a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">3.dưỡng</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">4.Bệnh Crohn</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">5.Bệnh cúm</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">6.Bệnh dại</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">7.Bệnh ghẻ</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">8.Bệnh gút</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">9.Bệnh hen phế quản</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">10.Bệnh hẹp van hai lá</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">11.Bệnh HIV</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">12.Bệnh ho</a></li>
                                </ul>
                            </div>
                            <div class="col-right">
                                <ul class="list">
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">1.Bệnh suy dinh </a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">2.Bệnh còi xương suy dinh </a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">3.dưỡng</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">4.Bệnh Crohn</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">5.Bệnh cúm</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">6.Bệnh dại</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">7.Bệnh ghẻ</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">8.Bệnh gút</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">9.Bệnh hen phế quản</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">10.Bệnh hẹp van hai lá</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">11.Bệnh HIV</a></li>
                                    <li><a href="{{url('tracuu-thuocnamtribenh-chitiet')}}">12.Bệnh ho</a></li>
                                </ul>
                            </div>
                        </div>
                    </article>
                </div>
                <div class="layout-right">
                    <div class="box-searchs">
                        <div class="head">
                            Tìm kiếm nhanh
                        </div>
                        <div class="search-q">
                            <form action="">
                                <input type="text" placeholder="Nội dung tìm kiếm">
                                <a href="" class="btn-search">Tìm kiếm</a>
                            </form>
                        </div>
                    </div>
                    <div class="box-utility">
                        <div class="list-utility">
                            <a href=""><img src="images/bg/bg_u1.png" alt="B1"></a>
                            <a href=""><img src="images/bg/bg_u2.png" alt="B1"></a>
                            <a href=""><img src="images/bg/bg_u3.png" alt="B1"></a>
                        </div>
                    </div>
                    <div class="box-hot">
                        <div class="head">
                            Tin nổi bật
                        </div>
                        <div class="item">
                            <ul class="list-news">
                                <li><a href="#">Cây thuốc quý</a></li>
                                <li><a href="#">Cây Bách Bệnh công. dụng và Cây mật nhân tăng cường sinh lý cho nam giới</a></li>
                                <li><a href="#">Công dụng của Giảo cổ lam</a></li>
                                <li><a href="#">Cây Bách Bệnh công dụng và cách dùng</a></li>
                                <li><a href="#">Cây thuốc quý</a></li>
                                <li><a href="#">Cây Bách Bệnh công. dụng và Cây mật nhân tăng cường sinh lý cho nam giới</a></li>
                                <li><a href="#">Công dụng của Giảo cổ lam</a></li>
                                <li><a href="#">Cây Bách Bệnh công dụng và cách dùng</a></li>
                            </ul>
                        </div>
                    </div>
                    <div class="box-hot">
                        <div class="head">
                            Cây thuốc - Vị thuốc
                        </div>
                        <div class="item">
                            <ul class="list-news">
                                <li><a href="#">Cây thuốc quý</a></li>
                                <li><a href="#">Cây Bách Bệnh công. dụng và Cây mật nhân tăng cường sinh lý cho nam giới</a></li>
                                <li><a href="#">Công dụng của Giảo cổ lam</a></li>
                                <li><a href="#">Cây Bách Bệnh công dụng và cách dùng</a></li>
                                <li><a href="#">Cây thuốc quý</a></li>
                                <li><a href="#">Cây Bách Bệnh công. dụng và Cây mật nhân tăng cường sinh lý cho nam giới</a></li>
                                <li><a href="#">Công dụng của Giảo cổ lam</a></li>
                                <li><a href="#">Cây Bách Bệnh công dụng và cách dùng</a></li>
                            </ul>
                        </div>
                    </div>
                    <div class="box-hot">
                        <div class="head">
                            Chủ đề bệnh gan
                        </div>
                        <div class="item">
                            <ul class="list-news">
                                <li><a href="#">Các bệnh viêm gan</a></li>
                                <li><a href="#">Bệnh gan nhiễm mỡ</a></li>
                                <li><a href="#">Men gan tăng cao</a></li>
                                <li><a href="#">Bệnh ung thư gan</a></li>
                                <li><a href="#">Cây thuốc quý</a></li>
                                <li><a href="#">Cơ bản về sơ gan</a></li>
                                <li><a href="#">Sản phẩm giải độc gan</a></li>
                            </ul>
                        </div>
                    </div>
                    <div class="box-form-email">
                        <div class="head">
                            Đăng ký nhận tin
                        </div>
                        <div class="item">
                            <form action="">
                                <p>
                                    Bạn là người luôn quan tâm đến thông tin y tế sức khỏe? Hãy đăng ký Email để được nhận tin từ Tuệ Linh mỗi ngày
                                </p>
                                <input type="text" placeholder="Email của bạn">
                                <a href="" class="more">Gửi</a>
                            </form>
                        </div>
                    </div>
                    <div class="box-videos">
                        <div class="head">
                            Video
                        </div>
                        <div class="item">
                            <div class="videoBoxIn">
                                <div class="videoBoxInObject">
                                    <iframe width="280" height="315" src="https://www.youtube.com/embed/gvCaHL0V-6A" frameborder="0" allowfullscreen></iframe>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="box-social">
                        <div class="head">
                            Social
                        </div>
                        <div class="item">
                            <img src="images/example/social.jpg" alt="Social" width="300" height="274">
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <section class="box-slide-foot">
            <div class="fix">
                <h2 class="title wow bounce-in-left" data-wow-duration="1s">sản phẩm Tuệ Linh</h2>
                <div class="box-slide-products">
                    <div class="owl-carousel" id="box-slide-foot">
                        <div class="item">
                            <a href="" title="">
                                <img src="images/example/pr01.jpg" width="140" height="167" alt=""/>
                            </a>
                            <div class="info-foot">
                                <h3>Dầu gấc Tuệ Linh</h3>
                                <p class="des">Dầu gấc nguyên chất</p>
                            </div>
                        </div>
                        <div class="item">
                            <a href="" title="">
                                <img src="images/example/pr01.jpg" width="140" height="167" alt=""/>
                            </a>
                            <div class="info-foot">
                                <h3>Dầu gấc Tuệ Linh</h3>
                                <p class="des">Dầu gấc nguyên chất</p>
                            </div>
                        </div>
                        <div class="item">
                            <a href="" title="">
                                <img src="images/example/pr01.jpg" width="140" height="167" alt=""/>
                            </a>
                            <div class="info-foot">
                                <h3>Dầu gấc Tuệ Linh</h3>
                                <p class="des">Dầu gấc nguyên chất</p>
                            </div>
                        </div>
                        <div class="item">
                            <a href="" title="">
                                <img src="images/example/pr01.jpg" width="140" height="167" alt=""/>
                            </a>
                            <div class="info-foot">
                                <h3>Dầu gấc Tuệ Linh</h3>
                                <p class="des">Dầu gấc nguyên chất</p>
                            </div>
                        </div>
                        <div class="item">
                            <a href="" title="">
                                <img src="images/example/pr01.jpg" width="140" height="167" alt=""/>
                            </a>
                            <div class="info-foot">
                                <h3>Dầu gấc Tuệ Linh</h3>
                                <p class="des">Dầu gấc nguyên chất</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    </section><!--//box-solution-->
@endsection